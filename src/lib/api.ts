...
async function fetchWithRetry(url: string, options: RequestInit = {}, retries = 2): Promise<Response> {
  ...
  try {
    ...
    if (retries > 0 && error instanceof Error && !error.name.includes("AbortError")) {
      console.warn(`🔄 Reintentando fetch (${retries} intentos restantes):`, error.message)
...
export async function getCategoria(codigoCategoria: string, isPvc = false): Promise<ApiResponse<Categoria>> {
  try {
    ...
    const foundCategoria = categorias.find((cat) => cat.codigo === codigoCategoria)

    if (foundCategoria) {
      return { data: foundCategoria, error: null }
    } else {
      return { data: null, error: `Categoría con código ${codigoCategoria} no encontrada` }
    }
  } catch (err) {
    const errorMessage = err instanceof Error ? err.message : "Error desconocido"
    return { data: null, error: errorMessage }
...
export async function getProductos(codigoCategoria: string): Promise<ApiResponse<Producto[]>> {
  try {
    ...
    if (productosData.length > 0) {
      ...
      return { data: [], error: "No se encontraron productos en esta categoría" }
    }
  } catch (err) {
    const errorMessage = err instanceof Error ? err.message : "Error desconocido"
    console.error(`❌ Error al obtener productos de ${codigoCategoria}:`, errorMessage)
    return { data: null, error: errorMessage }
  }
}

// Obtener un producto específico
export async function getProducto(codigoCategoria: string, codigoProducto: string): Promise<ApiResponse<Producto>> {
  try {
    ...
    if (producto) {
      ...
      return { data: null, error: `Producto con código ${codigoProducto} no encontrado` }
    }
  } catch (err) {
    const errorMessage = err instanceof Error ? err.message : "Error desconocido"
    return { data: null, error: errorMessage }
